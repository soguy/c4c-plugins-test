define({
	...
	onBeforeTemplateGenerate: function(templateZip, model) {
	    var templateType = utils.getTemplateType(templateZip, model),
	        newTemplateZip,
	        newModel;
	   
	    if (templateType === "Shared") {
	        newModel = jQuery.extend(true, {}, model);
	        newTemplateZip = utils.createSharedResources(templateZip, newModel);
	    } else if(templateType === "Translation"){
	               newTemplateZip = utils.createTranslation(templateZip, model);  
	               newModel = {};
    	    } else {
    	        newModel = utils.regenerateModel(templateType,templateZip, model);
    	        newModel.avilableBO = newModel.contentModel.avilableBO
    	        newTemplateZip = utils.regenerateZip(templateType,templateZip, newModel);
    	    }
